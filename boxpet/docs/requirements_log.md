...
- 涉及：`pet_def.h`（速率/门槛）、`pet.cpp`（SLEEPING 判定/toggle_light 返回码/删 force_wake/
  新增 in_sleep_window_now、seconds_to_energy_full）、`power_mgr.*`（Nap 原因/续睡判定）、
...
- **玩法**（ui_game `Mode::Counter`）：左右键在 9 位间切换选中位（个→十→百→千→万→十万→百万→千万→亿），
