...
### 迭代 M19 —— 复位提示游戏化（穿越/重生）

- **看门狗复位 → 「从异世界穿越回来了！」**：显示提示后触发进化光效演出
  （白色光晕闪烁 + 形态交替）+ Evolve 音效，宠物看起来"穿越后变了个样"。
- **程序崩溃 → 「宠物重生了！」**：显示提示后宠物**等级 -1**（经验清零）、
  **日龄退回上一阶段起始日**（完全体→成熟期起点，成熟期→成长期起点），
  并立即存 NVS 保持退回状态。
- 其余复位提示不变（欠压/USB/电源毛刺/挂死自愈）。
- 新增 `ui_main_trigger_evolve_fx()` 公开接口（进化光效 + Happy 动画，主循环调用）。
- 字库补 宠/穿/界/越/从 5 字并重新生成 `ui_font_16.c`。

### 迭代 M18 —— 宠物形象重设计 + 摸宠物反应 + 网络对时 + 生命周期/相亲调整

- **宠物形象重设计（v6，16 形态）**：`tools/sprite_gen2.py` 改为参数化
  `draw_form(stage, branch, pose)` ——
  - **阶段=体型/比例差异**：幼生（单团子头身合一）→ 成长（头身分离+脖子+细四肢）
    → 成熟（成人比例+装备齐备）→ 完全体（最大体格+传奇元素）；
  - **分支=配色+体型+职业装备**：力量=橙·宽扁壮实（红头带/护腕/大拳套/金腰带）、
    魔法=**淡紫**·瘦高（呆毛/额星/尖顶法师帽/法杖/星环长袍）、速度=绿·修长
    （尖耳/尾巴/围巾/闪电纹/风翼）；
  - **动作/表情帧与形态一致**：开心/进食/睡觉(Zzz)/生病(X眼)/委屈(泪滴) 均由
    同一剪影生成，属性<60 时不再变回"小圆球"。
- **摸宠物反应系统**：按当前状态给出相符反应 —— 睡觉=眯眼轻哼（不吵醒）、
  生病=虚弱、抑郁/心情低/饱食低=委屈、卫生低=蔫、精力低=犯困；
  状态良好时随机三选一（点头 / **左右蹭（新增 `AnimAction::Pat`）** / 高兴弹跳），
  并播放**仓鼠式开心叫声**（新增 `Sound::Squeak`：三声短促高音，偶补一声更高）。
- **网络对时（NTP）**：天气查询联网成功后顺手 SNTP 校时（ntp.aliyun.com，8s 超时，
  失败不影响天气），按东八区 +8h 校准墙钟并立即持久化（`wallclock_sync_to_epoch`）。
  用于消除深睡期间 RTC 慢时钟漂移的累积误差（每 2h + 每次开机各校一次）。
  墙钟 offset 加自旋锁（补跳/调时/NTP/定时器四路并发，防 int64 撕裂）。
- **事件提示队列化**：宠物 tick（esp_timer 任务）不再调用 `show_toast`（碰 LVGL），
  改为入队由独立 UI tick 显示 —— 针对"深睡醒来补跳后叠加事件"偶发看门狗/僵死。
- **生命周期与相亲调整**：
  - 演示模式寿命压缩：老年门槛 60 → **20 宠物日**、老年寿终概率 5% → **25%/日**
    （真实模式不变）；
  - **相亲等级门槛 15 → 4**（宠物进化满级仅 5，原门槛不可达）；
  - 设置页新增调试项「**立刻死亡**」（一键验证死亡界面 + 长按中键孵新蛋）；
  - 修正死亡场景判断：原判断 `stage==Dead`（无人设置→死代码）改为按 `pstate`，
    且**仅主界面触发**（否则在设置页死亡会卡住该页面、长按返回失效）。
- 字库补 立/刻/晴/阴/雪/雷/阵/雾/° 等字并重新生成 `ui_font_16.c`。
